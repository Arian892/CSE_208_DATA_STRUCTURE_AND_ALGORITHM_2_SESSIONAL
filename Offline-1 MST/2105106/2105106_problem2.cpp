...
class Graph
{
    ...
    vector<vector<int>> countinmst;   // for storing the in how many mst an edge is included
    int minimum_weight;
    int mstsize;
    int edges, nodes;
...
public:
    ...
    Graph(int nodes, int edges)
    {
        ...
        for (int i = 0; i <= nodes; i++)
            countinmst[i].resize(nodes + 1);  // resizing according to nodes
    ...
    void addEdge(Edge edge)
    {
        ...
        countinmst[edge.start][edge.end] = 0;    // reseting all zero
    }
    int find(int i)
    {
        if (parent[i] == -1)
    ...
    void increment(vector<Edge> check)  /// if mst found then all the edges of the mst increased by 1
    {
        for (auto edge : check)
        {
