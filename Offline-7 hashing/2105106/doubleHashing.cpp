...
public:
    ...
    void insert(string key, int value)
    {
        ...
        int probe = 0;
        int index = doubleHash(key, probe);
    ...
    unsigned int doubleHash(string &str, int i)
    {
        unsigned int hash1 = hashFunction(str);
        unsigned int hash2 = auxHash(str);

        return (hash1 + i * hash2) % maxSize;
    }
    ...
    element *find(string key)
    {
        ...
        int index = doubleHash(key, probe);
       // cout << index  << endl ;
        element *current = hashelement[abs(index)];
       // cout << "hello" ;
    ...
    void rehashing()
    {
        ...
        for (int i = 0; i < prevsize; i++)
        {
            ...
            {
                int probe = 0;
                int index = doubleHash(hashelement[i]->key, probe);
                while (temp[index] != nullptr && temp[index]->isDeleted == false)
                {
                    probe++;
                    index = doubleHash(hashelement[i]->key, probe);
                }
